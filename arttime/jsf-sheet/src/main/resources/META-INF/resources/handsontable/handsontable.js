            ...
            var rangeModifier = event.shiftKey ? selection.setRangeEnd : selection.setRangeStart;
              ...
              switch (event.keyCode) {
                ...
                case keyCodes.DELETE:
                  ...
                  that.prepareEditor();
                  event.preventDefault();
                  $( instance.rootElement ).trigger( "change" );
                  break;
...
(function (Handsontable) {
  ...
  ContextMenu.prototype.createMenu = function (menuName, row) {
	if (this.instance.getSelectedRange() === undefined) return; 
    if (menuName) {
...
//      }
  ...
  ContextMenu.prototype.show = function (menu, items) {
    ...
    var root = this.instance.rootElement[0];    
    if (root.addEventListener) {
    	root.addEventListener('DOMMouseScroll', wheel, false);
    }
  ...
  ContextMenu.prototype.close = function (menu) {
	...
	if (root.removeEventListener) {
		root.removeEventListener('DOMMouseScroll', wheel, false);
    }
	root.onmousewheel = null;
  };
  ...
  var doSaveComment = function (row, col, comment, instance) {
    ...
    bindMouseEvent = function (range) {
	  ...
	  function commentsListener(event) {
        ...
        if (!(event.target.className == 'htCommentTextArea' || event.target.innerHTML.indexOf('Comment') != -1 || event.target.innerHTML.indexOf('комментарий') != -1)) {
    ...
    placeCommentBox = function (range, commentBox) {
      ...
      var div = $(instance.rootElement);
      commentBox.style.position = 'absolute';
      commentBox.style.left = offset.left + lastColWidth - div.offset().left + 'px';
      commentBox.style.top = offset.top - div.offset().top + 'px';
    ...
    createCommentBox = function (value) {
      ...
      if (!comments) {
        ...
        $(instance.rootElement).append($(comments));
      }
  ...
  addCommentsActionsToContextMenu = function (defaultOptions) {
    ...
    defaultOptions.items.push({
      ...
      callback: function (key, selection, event) {
          var commentBox = Handsontable.Comments.showComment(this.getSelectedRange());
          setTimeout(function() {
              commentBox.getElementsByTagName('textarea')[0].focus();
              instance.unlisten();
          }, 100);
      },
      disabled: function () {
